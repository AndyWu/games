...
- A worm spawns on one of the world's trees the first time anyone loads the shared world. It eats a nearby leaf block once every 2 in-game hours (5 real minutes, since a full in-game day is 1 real hour — a genuine, permanent world edit, same as if you'd broken it yourself) and has 2 children near itself once every 24 in-game hours (one full in-game day, i.e. 1 real hour), so the population grows steadily over a play session and keeps on eating (capped at 100 so it can't run away entirely). Standing in an active fire kills it instantly, same as it would a player or animal. Worms and their eat/breed timers are saved and shared through the same multiplayer connection as everything else, so everyone sees the same worms and the population keeps growing even across reloads — in solo/offline play (no multiplayer connection) a single worm still spawns each session but doesn't persist. The current worm count is shown live in the HUD.
