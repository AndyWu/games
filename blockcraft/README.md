...
- A minimap in the top-left corner shows the whole (fixed-size) world from above — terrain colored the same as its blocks, so lakes read as blue and beaches as sand — with your own position as a small arrow pointing the way you're facing, and every other connected player as a colored dot in the same tint as their in-world character. It updates live and reflects any block you (or anyone else) build or dig, not just the original generated terrain.
