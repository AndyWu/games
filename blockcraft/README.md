...
- A minimap in the top-left corner shows the whole (fixed-size) world from above — terrain colored the same as its blocks, so lakes read as blue and beaches as sand — with your own position marked as a star and everyone else's as a colored triangle in the same tint as their in-world character, pointing whichever way they're actually facing. Both are labeled with names. It updates live and reflects any block you (or anyone else) build or dig, not just the original generated terrain.
- There are 4 seasons (Spring, Summer, Fall, Winter), each 3 real hours long (a full year is 12 hours), also derived from the system clock so everyone's on the same one. Each season has an average temperature — Spring 50°F, Summer 90°F, Fall 50°F, Winter 20°F — shown in the HUD, which then swings warmer at noon and colder at midnight and wobbles a little on its own, so it's never exactly the same twice. Standing outside (no roof, cave ceiling, or tree canopy overhead) in a genuinely dangerous stretch — a winter night or a summer noon are the ones to watch for — drains HP faster than standing still can regenerate it, with a pulsing red warning in the HUD; find shade, a cave, or a building and you're completely safe regardless of how extreme it gets outside.
