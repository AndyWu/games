...
- A worm spawns on one of the world's trees at load. It eats a nearby leaf block once every 2 real hours (a genuine, permanent world edit, same as if you'd broken it yourself) and has 2 children near itself once every 24 real hours, so the population slowly grows over time and keeps on eating (capped at 100 so it can't run away entirely). Standing in an active fire kills it instantly, same as it would a player or animal. The current worm count is shown live in the HUD.
